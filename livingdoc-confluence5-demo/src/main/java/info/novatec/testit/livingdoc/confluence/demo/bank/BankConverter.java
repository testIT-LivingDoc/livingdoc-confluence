    ...
    public Object parse(String value, Class<?> type) {
        ...
        for (int i = 0; i < numberOfAccounts; i++) {
            ...
            if (accountType.equals("SavingsAccount")) {
                accounts.put(number, (SavingsAccount) new SavingsAccountConverter().parse(jsonAccount.toString(), SavingsAccount.class));
            } else if (accountType.equals("CheckingAccount")) {
                accounts.put(number, (CheckingAccount) new CheckingAccountConverter().parse(jsonAccount.toString(), SavingsAccount.class));
            }
        }
