...
public class ConfluenceXmlRpcLivingDocServiceImpl implements LivingDocRpcHelper {
    ...
    public Vector< ? > getSpecificationHierarchy(final String username, final String password, final Vector< ? > args) {
        ...
        TransactionTemplate txTemplate = ldUtil.getTransactionTemplate();
        return ( Vector< ? > ) txTemplate.execute(new TransactionCallback<Object>() {
            @Override
            public Object doInTransaction() {
                ConfluenceUser user = null;
                ...
                try {
                    ...
                    return new DocumentNode(ldUtil.getText(INVALID_SESSION)).marshallize();
                } catch (EntityException e) {
    ...
    private void checkPermissions(Space space, ConfluenceUser user) throws InvalidSessionException, NotPermittedException {
    ...
    private ConfluenceUser login(String username, String password) throws RemoteException, EntityException {
        if (StringUtils.isEmpty(username))
            return null;
        ConfluenceUser confUser = null;
        if (ldUtil.isCredentialsValid(username, password)) {
            confUser = ( ConfluenceUser ) ldUtil.getConfluenceUserManager().getUser(username);
        }

        return confUser;
    }

    private void logout(ConfluenceUser user) {
    ...
    private String getRenderedSpecification(Page page, boolean implementedVersion, boolean includeStyle) {
        try {
            ...
            basicRenderedPage.append("<meta http-equiv=\"content-type\" content=\"text/html;charset=").append(ldUtil
                .getEncoding()).append("\"/>\n");
            ...
            // basicRenderedPage.append(ldUtil.getWikiStyleRenderer().convertWikiToXHtml(page.toPageContext(),
            // content));
            basicRenderedPage.append(ldUtil.getViewRenderer().render(content, new DefaultConversionContext(page
                .toPageContext())));
    ...
    private DocumentNode buildNodeHierarchy(Page page, ConfluenceLivingDoc ldUtil) {
        DocumentNode node = new DocumentNode(page.getTitle());
        node.setCanBeImplemented(ldUtil.canBeImplemented(page));

        List<Page> children = page.getChildren();
        for (Page child : children)
            node.addChildren(buildNodeHierarchy(child, ldUtil));
    ...
    private String warning(String errorId) {
        ...
        sb.append("<html>");
        sb.append(
            "  <table style=\"text-align:center; border:1px solid #FFD700; border-spacing:0px; background-color:#FFFF66; padding:0px; margin:5px; width:100%;\">");
        sb.append("    <tr style=\"display:none\"><td>Comment</td></tr>");
        sb.append(
